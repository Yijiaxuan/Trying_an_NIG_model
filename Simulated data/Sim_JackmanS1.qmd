...
if (nrow(filtered) == 0) stop("No valid polling row for ALP")
...
write.csv(sim_df,
          file = "C:/Users/16438/Desktop/Reaserch Project/Simulated/simulated_S1.csv",
          row.names = FALSE)
...
plot_simulated_trend <- function(sim_df, start_date, target_last_y = 0.3334) {
  g <- ggplot(sim_df, aes(x = as.Date(mid_date))) +
    ...
    #annotate("text",
             ...
             #hjust = 1) +
    labs(
      #title = "Simulated Polling Data",
      #subtitle = "Constant latent trend; final poll = 0.3334",
      x = "",
      y = "Vote share (%)"
    ) +
    scale_x_date(
...
library(rstan)
library(tibble)
dataS1 <- read.csv("C:/Users/16438/Desktop/Reaserch Project/Simulated/simulated_S1.csv")
# build discontinuity index from dates 
.make_discontinuity <- function(date_seq, break_dates = NULL) {
  ND <- length(date_seq)
  if (is.null(break_dates) || length(break_dates) == 0) {
    return(list(NBREAKS = 0L, discontinuity = rep(0L, ND)))
...
xi_summary1 <- function(fit, start_date) {
  xi_samples1 <- rstan::extract(fit)$xi 
  xi_df1 <- apply(xi_samples1, 2, function(x) {
    tibble(
      mean = mean(x),
      q05  = quantile(x, 0.05),
      q95  = quantile(x, 0.95),
...
xi_df1 <- xi_summary1(fit1, start_date = as.Date("2016-07-02"))

```

```{r}
last_idx1 <- nrow(xi_df1)       
idx_42_1 <- last_idx1 - 41         

xi_df1[c(idx_42_1, last_idx1), c("date", "mean", "q025", "q975")]
...
last_poll <- filtered_data1 %>% arrange(mid_date) %>% slice_tail(n = 1)
...
g_trend1 <- plot_trend(
  xi_df = xi_df1,
  polls_df = filtered_data1 %>% mutate(y = y * 100),
  party_label = "ALP"
)
print(g_trend1)
```
...
end_date_win1   <- max(xi_df1$date)
start_date_win1 <- end_date_win1 - days(41)

xi_last42_1 <- xi_df1 %>%
  filter(date >= start_date_win1 & date <= end_date_win1)

polls_last42_1 <- filtered_data1 %>%
  filter(mid_date >= start_date_win1 & mid_date <= end_date_win1) %>%
  mutate(y = y * 100)   


g_trend_last42_1 <- ggplot(xi_last42_1, aes(x = date)) +
  geom_ribbon(aes(ymin = q025 * 100, ymax = q975 * 100),
              fill = "grey80", alpha = 0.6) +
  geom_line(aes(y = mean * 100), linewidth = 1.2) +
  geom_point(data = polls_last42_1,
             aes(x = mid_date, y = y),
             inherit.aes = FALSE, alpha = 0.7, size = 1.4) +
  geom_vline(xintercept = as.numeric(end_date_win1), linetype = 3) +
  annotate("text", x = end_date_win1, y = Inf, label = "Last day",
...
print(g_trend_last42_1)

```

```{r}
delta_array1 <- tryCatch(
  rstan::extract(fit1, pars = "delta")$delta, 
  error = function(e) NULL
)

if (is.null(delta_array1)) {

  delta_raw_samps1 <- rstan::extract(fit1, pars = "delta_raw")$delta_raw 
  delta_array1 <- delta_raw_samps1 - rowMeans(delta_raw_samps1)
}

delta_summary1 <- apply(delta_array1, 2, function(x) {
  c(mean = mean(x), lo = quantile(x, 0.05), hi = quantile(x, 0.95))
}) %>% t() %>% as.data.frame() %>% tibble::rownames_to_column("house_idx")


house_levels1 <- levels(factor(filtered_data1$pollster))
if (length(house_levels1) == nrow(delta_summary1)) {
  delta_summary1$pollster <- house_levels1
} else {

  map_tbl1 <- filtered_data1 %>%
    distinct(j, pollster) %>%
    arrange(j)
  delta_summary1 <- delta_summary1 %>%
    mutate(j = as.integer(house_idx)) %>%
    left_join(map_tbl1, by = c("j" = "j")) %>%
    mutate(pollster = ifelse(is.na(pollster), paste0("House ", j), pollster))
}

g_house1 <- delta_summary1 %>%
  mutate(
    pct = mean * 100,
    lo  = `lo.5%`  * 100,
    hi  = `hi.95%` * 100
